...
func SchoolOnly(c *fiber.Ctx) error {
	...
	if role != "school" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"status": "error", "message": "Hanya akun sekolah yang dapat mengakses fitur ini"})
	}
	return c.Next()
}
